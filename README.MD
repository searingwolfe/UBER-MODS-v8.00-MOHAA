...
| find | 'find truckclip' = teleport all players to $truckclip (or any other targetname, not including the $). <br> <br> 'find endtrigger' = teleport all players to $endtrigger. <br> <br> 'find aispawn' = teleport all players to $aispawn. <br> <br> 'find barrel[1]' = teleport all players to $barrel[1] (or any other [index] value if 2+ entities share a targetname). <br> <br> 'find barrel[12]' = teleport all players to $barrel[12]. | This teleports all players on the map to the $entity's coordinates (also printing: origin, angles, model, entnum, target, etc). <br> <br> This also works for finding simple entities that are NULL/don't exist as real entities, such as Actor path nodes, spawn points, and waypoints. |
